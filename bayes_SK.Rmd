...
$$  
with A = two of her children are found dead *due to SIDS (with no evidence of physical harm) and B = a mother is a son-murdering psychopath = 0.50. Assume that the rate of son-murdering psychopaths mothers is 1 in 1,000,000. According to Bayes' theorem, what is the probability of $\mbox{Pr}(B \mid A)$ ? 

```{r}
Pr(A) = 1/850000
Pr(B) = 1/10^6
Pr(B|A) = Pr(A|B) * Pr(B) / Pr(A) = 0.50 * 1/10^6 / (1/850000)
0.50 * (1/10^6) / (1/850000)
```
...
```{r}
results <- polls |> summarize(average = mean(spread), se = sd(spread) / sqrt(n()))
results
```
