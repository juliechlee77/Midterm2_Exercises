...
as.data.frame(fit$finalModel$means) |>
  mutate(type = rownames(fit$finalModel$means)) |>
...
#sp1, oaz2
#BUT ANSWER IN LAB WAS TGRBR3 AND F11R
```
...
**ANSWER:**
```{r}
#im not getting same values but whatever??
#plot(x[,"TGFBR3"], x[,"F11R"], col=y)
#not a very clear bivariate distribution -- still mixed, but there is still difference between two
# cannot clearly separate though, so not amazing predictor
```
...
#endometrium and kidney has low sensitivity, rest are pretty good 
```
