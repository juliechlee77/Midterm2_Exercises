...
top
left_join(top |> select(playerID, HR), People |> select(playerID, nameFirst, nameLast))
```
...
top <- top |> right_join(salaries, by = "playerID") |>
...
```{r}
co2_new <- co2_wide |> left_join(yearly_avg, by = "year") |> mutate(residuals = co2 - avg)
```

5\. Make a plot of the seasonal trends by year but only after removing the year effect. 
```{r}
co2_new |> ggplot() +
  geom_line(aes(month, residuals, color = as.factor(year)))
```
