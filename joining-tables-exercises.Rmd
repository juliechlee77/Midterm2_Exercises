...
```{r}
tab1 <- left_join(top, People, by = "playerID") |>
...
```{r}
top |> right_join(Salaries, top, by = "playerID") |>
  filter(yearID == 2016) |>
...
yearly_avg <- co2_wide |> group_by(year) |>
  select(month, co2) |>
  summarise(unique(year), mean(co2))
```

```{r}
names(yearly_avg)[3]=paste("co2")
yearly_avg
```
...
dfq4 <- left_join(co2_wide, yearly_avg, by = "year")
...
```{r}
dfq4 |> 
ggplot(aes(as.integer(year), co2.x, color = month)) + geom_line()
```
