...
```{r}
regression_prediction <- replicate(100, {
  y <- dat$y 
  test_index <- createDataPartition(y, times = 1, p = 0.5, list = FALSE)
  train_set <- dat |> slice(-test_index) 
  test_set <- dat |> slice(test_index) 
...
histogram(regression_prediction)
```
...
t <- function(n){
  MASS::mvrnorm(n = n, c(69, 69), Sigma) |> 
  data.frame() |> setNames(c("x", "y")) 
}
...
regression_prediction100 <- replicate(100, {
  y <- dat[[1]]$y 
  test_index <- createDataPartition(y, times = 1, p = 0.5, list = FALSE)
  train_set <- dat[[1]] |> slice(-test_index) 
  test_set <- dat[[1]] |> slice(test_index) 
  fit <- lm(y ~ x, data = train_set) 
  y_hat <- fit$coef[1] + fit$coef[2]*test_set$x 
  RMSE = sqrt(mean((y_hat - test_set$y)^2)) 
})
mean(regression_prediction100)
sd(regression_prediction100)

regression_prediction500 <- replicate(100, {
  y <- dat[[2]]$y 
  test_index <- createDataPartition(y, times = 1, p = 0.5, list = FALSE)
  train_set <- dat[[2]] |> slice(-test_index) 
  test_set <- dat[[2]] |> slice(test_index) 
  fit <- lm(y ~ x, data = train_set) 
  y_hat <- fit$coef[1] + fit$coef[2]*test_set$x 
  RMSE = sqrt(mean((y_hat - test_set$y)^2)) 
})
mean(regression_prediction500)
sd(regression_prediction500)

regression_prediction1000 <- replicate(100, {
  y <- dat[[3]]$y 
  test_index <- createDataPartition(y, times = 1, p = 0.5, list = FALSE)
  train_set <- dat[[3]] |> slice(-test_index) 
  test_set <- dat[[3]] |> slice(test_index) 
  fit <- lm(y ~ x, data = train_set) 
  y_hat <- fit$coef[1] + fit$coef[2]*test_set$x 
  RMSE = sqrt(mean((y_hat - test_set$y)^2)) 
})
mean(regression_prediction1000)
sd(regression_prediction1000)

regression_prediction5000 <- replicate(100, {
  y <- dat[[4]]$y 
  test_index <- createDataPartition(y, times = 1, p = 0.5, list = FALSE)
  train_set <- dat[[4]] |> slice(-test_index) 
  test_set <- dat[[4]] |> slice(test_index) 
  fit <- lm(y ~ x, data = train_set) 
  y_hat <- fit$coef[1] + fit$coef[2]*test_set$x 
  RMSE = sqrt(mean((y_hat - test_set$y)^2)) 
})
mean(regression_prediction5000)
sd(regression_prediction5000)

regression_prediction10000 <- replicate(100, {
  y <- dat[[5]]$y 
  test_index <- createDataPartition(y, times = 1, p = 0.5, list = FALSE)
  train_set <- dat[[5]] |> slice(-test_index) 
  test_set <- dat[[5]] |> slice(test_index) 
  fit <- lm(y ~ x, data = train_set) 
  y_hat <- fit$coef[1] + fit$coef[2]*test_set$x 
  RMSE = sqrt(mean((y_hat - test_set$y)^2)) 
})
mean(regression_prediction10000)
sd(regression_prediction10000)
```


3\. Describe what you observe with the RMSE as the size of the dataset becomes larger. 
a. On average, the RMSE does not change much as `n` gets larger, while the variability of RMSE does decrease. -> Correct
...
c. When we increase the correlation between `x` and `y`, `x` has more predictive power and thus provides a better estimate of `y`. This correlation has a much bigger effect on RMSE than `n`. Large `n` simply provide us more precise estimates of the linear model coefficients.  _> Correct
...
y_hatx1 <- fitx1$coef[1] + fitx1$coef[2]*test_set$x_1 
sqrt(mean((y_hatx1 - test_set$y)^2)) 

#just x_2
fitx2 <- lm(y ~ x_2, data = train_set) 
y_hatx2 <- fitx2$coef[1] + fitx2$coef[2]*test_set$x_2 
sqrt(mean((y_hatx2 - test_set$y)^2)) 

#x_1 and x_2
fitx12 <- lm(y ~ x_1 + x_2, data = train_set) 
y_hatx12 <- fitx12$coef[1] + fitx12$coef[2]*test_set$x_1 + fitx12$coef[3]*test_set$x_2
sqrt(mean((y_hatx12 - test_set$y)^2)) 
```
for both x_1 and x_2, the RMSE is smaller
...
a. Adding extra predictors can improve RMSE substantially, but not when they are highly correlated with another predictor.  -> correct
...
```{r}
fitglm <- glm(y ~ x, data = dat$train, family = "binomial") 
y_hatlinear <- fitlinear$coef[1] + fitlinear$coef[2]*dat[[2]]$x 
p_hat_glm <- predict(fitglm, dat$test, type="response")
y_hat_glm <- factor(ifelse(p_hat_glm > 0.5, 1, 0))
confusionMatrix(y_hat_glm, dat$test$y)$overall["Accuracy"]

lm_fit <- mutate(dat$train, y = as.numeric(y)) |> 
  lm(y ~ x, data = _)
p_hat <- predict(lm_fit, dat$test)
y_hat <- ifelse(p_hat > 0.5, "0", "1") |> factor()
confusionMatrix(y_hat, dat$test$y)$overall[["Accuracy"]]

```

logistic regression higher frequency
...
```{r}
logistic_prediction <- replicate(100, {
  fitglm <- glm(y ~ x, data = dat$train, family = "binomial") 
  y_hatlinear <- fitlinear$coef[1] + fitlinear$coef[2]*dat[[2]]$x 
  p_hat_glm <- predict(fitglm, dat$test, type="response")
  y_hat_glm <- factor(ifelse(p_hat_glm > 0.5, 1, 0))
...
mean(logistic_prediction)


linear_prediction <- replicate(100, {
  lm_fit <- mutate(dat$train, y = as.numeric(y)) |> 
  lm(y ~ x, data = _)
  p_hat <- predict(lm_fit, dat$test)
  y_hat <- ifelse(p_hat > 0.5, "0", "1") |> factor()
