...
train_lm2 = function(x) {
  ...
  c(mean = mean(train_lm), sd = sd(train_lm))
}
...
mean(train_lm3)   #mean(RMSE) 2.66 -> 1.01
sd(train_lm3)     #sd(RMSE)  0.169 -> 0.0712
hist(train_lm3)   
```
...
y_hat1 = fit1$coef[1] + fit1$coef[2]*test_set$x_1
RMSE1 = sqrt(mean((y_hat1 - test_set$y)^2)) 

#x_2
fit2 = lm(y ~ x_2, data = train_set) 
y_hat2 = fit2$coef[1] + fit2$coef[2]*test_set$x_2
RMSE2 = sqrt(mean((y_hat2 - test_set$y)^2)) 


#x_1 and x_2 
fit12 = lm(y ~ x_1 + x_2, data = train_set) 
y_hat12 = fit12$coef[1] + fit12$coef[2]*test_set$x_1 + fit12$coef[3]*test_set$x_2
RMSE12 = sqrt(mean((y_hat12 - test_set$y)^2)) 

c(RMSE1 = RMSE1, RMSE2 = RMSE2, RMSE12 = RMSE12)
```
...
logistic_25 = sapply(delta, function(d){
  dat = make_data(mu_1 = d)
  fit_glm = glm(y ~ x, data=dat$train, family = "binomial")
  y_hat_glm = ifelse(predict(fit_glm, dat$test) > 0.5, 1, 0) |>
    factor(levels = c(0, 1))         
  ##why does factor(ifelse(predict(fit_glm, dat$test) > 0.5, 1, 0)) not work??
  accuracy = mean(y_hat_glm == dat$test$y)
})

plot(delta, logistic_25)

linear_25 = sapply(delta, function(d){
  dat = make_data(mu_1 = d)
  p_hat_lm = predict(fit_lm, dat$test, type="response")
  y_hat_lm = ifelse(p_hat_lm > 0.5, 1, 0) |>
    factor(levels = c(0, 1))         
  mean(y_hat_lm == dat$test$y)
})

plot(delta, linear_25)

dd = data.frame(delta = delta, linear = linear_25, logistic = logistic_25) |> 
  pivot_longer(2:3, names_to = "method", values_to = "accuracy")
dd

dd |> ggplot(aes(delta, accuracy, color = method)) +
