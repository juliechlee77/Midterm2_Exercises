...
dat |> filter(!is.na(deaths)) |>
  ...
  geom_point() +  
  geom_smooth(method = loess, method.args = list(span = 60/as.numeric(diff(range(dat$date))), degree = 1))
...
#cannot use yday(date) bc it will repeat just like day

dat |> filter(!is.na(deaths)) |>
  mutate(smooth = fit$fitted) |>
  ggplot(aes(yday(date), smooth, color = year, group = year)) + 
  geom_line() 
