...
dat <- map_df(str_split(pdf_text(fn), "\n"), function(s){ 
  ...
  s[-out] |>  str_remove_all("[^\\d\\s]") |> str_trim() |> 
    str_split_fixed("\\s+", n = 6) |> .d(,1:5) |> as_tibble() |>  
...
dat <- dat |> mutate(x = as.numeric(date))
fit <- loess(deaths ~ x, data = dat, span = span, degree = 1)

dat |> mutate(smooth = predict(fit, as.numeric(date))) |>
  ggplot() +
  geom_point(aes(date, deaths)) +
  geom_line(aes(date, smooth), lwd = 1, col = 2)
```
...
fit <- loess(as.integer(y) ~ x_2, degree = 1, span = 0.5, data = mnist_27$train)
