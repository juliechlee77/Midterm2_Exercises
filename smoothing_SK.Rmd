...
# if there's a problem with new pipe |>, change to old pipe %>%
```{r, eval=FALSE} 
library(tidyverse) 
library(lubridate) 
library(purrr) 
...
dat |>
  ...
  geom_point() + 
  geom_smooth(method = 'loess', span = 60/as.numeric(diff(range(dat$date)))) #gives you confidence interval too
```
