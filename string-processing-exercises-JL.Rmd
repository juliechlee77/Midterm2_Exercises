...
c. It is a report combining graphs and tables. Extracting the data seems possible. -> correct
...
b. A character string of length 12. Each entry represents the text in each page. The mortality data is in there somewhere.  -> correct
...
d. I can see the table! But there is a bunch of other stuff we need to get rid of.-> correct
...
str(s)
```
vector of type charactor with 41 entries
...
str_split(s[-c(1:3)], pattern = '\\s+', simplify = TRUE)
```
...
```{r}
#str_remove_all(s, pattern = '^\\D.+')
#str_remove_all(s, pattern = '([a-z].+ |[A-Z].+)')
s <- str_remove_all(s, pattern = '[^\\d\\s]')
s
```
...
```{r}
library(tidyverse)
#s <- str_split_fixed(s, pattern = '\\s+', n=5)
#s <- s[,1:4] |> as.tibble()
#s
...
```{r}
dat <- st  %>% setNames(c("day","2015", "2016", "2017")) %>% mutate_all(as.numeric)
dat
```
...
s <- s[-c(1:header_index, tail_index:length(s), which (n==1))]
s <- str_remove_all(s, pattern = '[^\\d\\s]') |>
  str_split_fixed("\\s+",6) |> _[,1:4] |> as_tibble()
dat <- st  %>% setNames(c("day","2015", "2016", "2017")) %>% mutate_all(as.numeric)
tab <- dat |> pivot_longer(cols = c("2015", "2016", "2017"), names_to = "year", values_to = "death")
