...
**ANSWER:**
```{r}
header_index <- str_which(s, pattern = "[A-Z]\\s{2,}\\d{4}")
header_index
```
...
```{r}
#s[header_index]
#output: "SEP     2015    2016     2017    2018"
# month = "SEP" ;  header = 2015, 2016, 2017, 2018
header <- s[header_index] |>
  str_split("\\s{2,}", simplify = T)
header <- header[-1]
header

month <- s[header_index] |>
  str_split("\\s{2,}",simplify = T)
month <- month[1]
month
```
...
**ANSWER:**
```{r}
s <- s |> str_split_fixed(pattern = "\\s+", n = 6)
s <- s[,1:4] |> as_tibble()
s
```
...
**ANSWER:**
```{r}
tab <- pivot_longer(dat, cols = header[-4], names_to = "year", values_to = "deaths")
tab
```
...
header_index <- s |> str_which(pattern = "[A-Z]\\s{2,}\\d{4}")
tail_index <- s |> str_which(pattern = "Total")

.d <- `[` 
header <- s[header_index] |> str_split("\\s{2,}", simplify = T) |> .d(-1) #.d(2:5)
month <- s[header_index] |> str_split("\\s{2,}",simplify = T) |> .d(1) 

s <- s[(header_index + 1):(tail_index - 1)]
n <- s |> str_count(pattern = "\\d+")
s <- s[-which(n==1)]

s <- s |> str_remove_all(pattern = "[^\\d\\s]") |> 
  str_split_fixed(pattern = "\\s+", n = 6) |>
  .d(,1:4) |> as_tibble() |>
  setNames(c("day", header[-4])) |>
  mutate_all(as.numeric) |>
  pivot_longer(dat, cols = header[-4], names_to = "year", values_to = "deaths")
s
```
