...
library(pdftools) 
``` 
Now read-in `fn` using the `pdf_text` function and store the results in an object called `txt`.
```{r}
txt <- pdf_text(fn)
str(txt) #what type of data, how long, some first entries
```

Which of the following best describes what you see in `txt`? 
...
4\. Extract the ninth page of the PDF file from the object `txt`, then use the `str_split` from the __stringr__ package so that you have each line in a different entry. Call this string vector `s`. 
```{r}
library(stringr)
...
Then look at the result and choose the one that best describes what you see. 
...
str_split(s[-c(1:3)], pattern = '\\s+', simplify = T)
```
...
```{r}
a <- s[-c(1:header_index, tail_index:length(s), which(n==1))]
```
...
```{r}
b <- str_remove_all(a, pattern = "([A-Z].+)")
```
...
```{r}
library(tidyverse)
c <- b |> str_split_fixed(pattern = "\\s+", n = 6) |> .[,1:4] |> as_tibble()
```
...
```{r}
dat <- c |> setNames(c("Day", "2015", "2016", "2017")) |> mutate_all(as.numeric)
```


16\. Now finish it up by tidying `tab` with the `pivot_longer_ function. 
```{r}
tab <- dat |> pivot_longer(cols = c("2015", "2016", "2017"), names_to = "year", values_to = "death")
...
library(ggplot2)
tab |> ggplot() +
  geom_line(aes(x = day, y = death, col = year)) +
  ggtitle("Deaths versus Day") +
  theme_classic()
```
